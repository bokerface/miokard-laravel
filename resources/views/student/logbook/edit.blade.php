    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            <div class="table-responsive">
                <form action="{{ route('student.update_logbook', $logbook->id) }}" method="POST">
                    @method('PUT')
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="description" name="description" value="{{ old('description') }}"
                            class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" cols="30" rows="5">{{ old('description') ?? $logbook->description }}</textarea>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="patient_name" name="patient_name"
                            value="{{ old('patient_name') ?? $logbook->patient_name }}"
                        ...
                        <select id="patient_gender" name="patient_gender"
                            ...
                            @foreach ($genders as $gender)
                                <option value="{{ $gender->value }}"
                                    {{ old('patient_gender') == $gender->value ? 'selected' : ($logbook->patient_gender == $gender->value ? 'selected' : '') }}>
                                    {{ $gender->value }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="patient_age" name="patient_age"
                            value="{{ old('patient_age') ?? $logbook->patient_age }}"
                    ...
                    <div class="form-group">
                        <label for="type_of_action" class="col-form-label">Tindakan</label>
                        <textarea id="type_of_action" name="type_of_action" value="{{ old('type_of_action') }}"
                            class="form-control {{ $errors->has('type_of_action') ? 'is-invalid' : '' }}" cols="30" rows="5">{{ old('type_of_action') ?? $logbook->type_of_action }}</textarea>
